...
# Guardamos token en memoria (evita ir al disco cada vez)
cached_token = None
...
def obtener_token():
    """Obtiene un nuevo token desde el endpoint de autenticación."""
    global cached_token
    url = os.getenv("RNE_AUTH_URL")
    user = os.getenv("RNE_USER")
    password = os.getenv("RNE_PASS")

    try:
        response = requests.post(url, json={"usuario": user, "password": password}, timeout=10)
        if response.status_code == 200:
            token = response.json().get("token")
            if token:
                cached_token = token
                print("✅ Nuevo token obtenido y cacheado")
                return token
        print(f"⚠️ Error al obtener token: {response.status_code} {response.text}")
        return None
...
def get_token():
    """Devuelve el token cacheado o intenta generarlo si no existe."""
    global cached_token
    if not cached_token:
        cached_token = os.getenv("CRC_TOKEN") or obtener_token()
    return cached_token
...
def login():
    ...
    if user:
        ...
        cursor.execute(
            "UPDATE usuarios SET ultima_conexion = %s WHERE id = %s",
        ...
        # Añadir vendedor_id solo si aplica
        if user['tipo_usuario'] == 'vendedor':
            base_response['vendedor_id'] = user['id']
    ...
    try:
        query = """
            SELECT id, nombres, apellidos, identificacion, telefono, direccion, ciudad, correo,
                   ...
                   asignado_a, fecha_asignacion, seguimientos, test_drive,
                   capturado_por, habeas_data, aceptacion_tratamiento_datos,
                   firma_digital, concesionario_id, fecha_aceptacion,
        ...
        with conn.cursor() as cur:
            cur.execute(query, (int(vendedor_id),))
            rows = cur.fetchall()
            colnames = [desc[0] for desc in cur.description]

        print("Columnas devueltas:", len(colnames))
        print("Filas recuperadas:", len(rows))

        leads_list = []
        for row in rows:
            lead_dict = dict(zip(colnames, row))

            # Normalizar campos
            lead_dict["fecha_lead"] = str(lead_dict.get("fecha_lead")) if lead_dict.get("fecha_lead") else None
            lead_dict["fecha_asignacion"] = str(lead_dict.get("fecha_asignacion")) if lead_dict.get("fecha_asignacion") else None
            lead_dict["fecha_aceptacion"] = str(lead_dict.get("fecha_aceptacion")) if lead_dict.get("fecha_aceptacion") else None
            lead_dict["seguimientos"] = lead_dict.get("seguimientos") or ""
            lead_dict["test_drive"] = lead_dict.get("test_drive") or "No"
            lead_dict["precio_venta"] = float(lead_dict.get("precio_venta") or 0)
...
def delete_usuario():
    try:
        ...
        return jsonify({"message": f"Usuario '{user[1]}' eliminado exitosamente"}), 200
...
def exportar_reporte_gerente():
    ...
    try:
        ...
        # Filtrar por vendedor ID si corresponde
        vendedor_nombre_completo = None
        if vendedor_id:
        ...
        headers = [
            "Vendedor", "Nombres", "Apellidos", "Identificación", "Teléfono", "Dirección",
            "Ciudad", "Correo", "Fecha Lead", "Origen Lead", "Marca de Interés",
            "Modelo Interesado", "Precio de Venta", "Estatus", "Test Drive", "Seguimientos"
        ]
        ...
        # ✅ Generar Excel (.xlsx)
            ...
            for lead in leads:
                ws.append([
                    ...
                    lead.get('ciudad', ''),
                    lead.get('correo', ''),
                    str(lead.get('fecha_lead', '')).split(" ")[0],
                    lead.get('origen_lead', ''),
                    lead.get('marca_interes', ''),
                    lead.get('modelo_interesado', ''),
                    lead.get('precio_venta', ''),
                    lead.get('estatus', ''),
                    lead.get('test_drive', ''),
                    lead.get('seguimientos', '')
                ])
            ...
            response.headers['Content-Disposition'] = 'attachment; filename=reporte_leads.xlsx'
...
def get_leads_count_expert():
    ...
    try:
        ...
        print(f"🛠️ Usuario {user_id} - Fecha {fecha_actual} - Leads hoy: {count}")

        return jsonify({"count": count}), 200

    except Exception as e:
        conn.rollback()  # 👈 Resetea la transacción fallida
        print(f"❌ Error en get_leads_count_expert: {e}")
        return jsonify({"error": "Error al obtener la cantidad de leads", "details": str(e)}), 500
...
def consultar_rne():
    ...
    # 🔹 Cargar el token siempre desde el .env
    token = os.getenv("CRC_TOKEN")
    if not token:
        return jsonify({"error": "No se encontró CRC_TOKEN en el backend"}), 500
    ...
    consulta_data = {
        ...
        "consulta_tel": telefono,
        "consulta_corr": correo,
        "fecha_consulta": get_current_time_colombia(),
        "tipo": results["telefono"].get("tipo") if results["telefono"] else "NR",
        "sms": "S" if results["telefono"] and results["telefono"]["opcionesContacto"].get("sms") else "N",
        "aplicacion": "S" if results["telefono"] and results["telefono"]["opcionesContacto"].get("aplicacion") else "N",
        "llamada": "S" if results["telefono"] and results["telefono"]["opcionesContacto"].get("llamada") else "N",
        "fecha_creacion_tel": results["telefono"]["fechaCreacion"] if results["telefono"] else None,
        "aplicacion_corr": "S" if results["correo"] and results["correo"]["opcionesContacto"].get("aplicacion") else "N",
        "correo_corr": "S" if results["correo"] and results["correo"]["opcionesContacto"].get("correo_electronico") else "N",
        "fecha_creacion_corr": results["correo"]["fechaCreacion"] if results["correo"] else None,
    }

    try:
        cursor.execute("""
            INSERT INTO log_consultas_rne (
                vendedor_id, consulta_tel, fecha_consulta, tipo,
                sms, aplicacion, llamada, fecha_creacion_tel,
                consulta_corr, aplicacion_corr, correo_corr, fecha_creacion_corr
        ...
        conn.rollback()
        print(f"Error al insertar log RNE: {e}")
        return jsonify({"error": "Error al registrar log RNE"}), 500
        ...
        leads_list = [
            {
                ...
                "test_drive": lead[15],
                "seguimientos": lead[16] if lead[16] else "",
                "vendedor_id": lead[17] if lead[17] else None,
                "vendedor_nombre": lead[18],
                "precio_venta": float(lead[19]) if lead[19] is not None else 0
            }
            for lead in leads
        ]
        return jsonify(leads_list), 200
...
def update_lead_gerente():
    ...
    try:
        ...
        if data['estatus'] != "nuevo" and 'vendedor_id' in data:
            data['asignado_a'] = data['vendedor_id']
            data['asignado'] = True
        else:
            data['asignado_a'] = None
            data['asignado'] = False
        ...
        # 🔹 Incluimos precio_venta en el UPDATE
        query = """
            UPDATE leads
            SET nombres = %s, apellidos = %s, identificacion = %s, telefono = %s, direccion = %s, ciudad = %s, correo = %s, 
                origen_lead = %s, marca_interes = %s, modelo_interesado = %s, estatus = %s, asignado = %s, asignado_a = %s, 
                seguimientos = %s, test_drive = %s, precio_venta = %s,
                habeas_data = %s, aceptacion_tratamiento_datos = %s
            WHERE id = %s
        """
        params = [
            data['nombres'], data['apellidos'], data['identificacion'], data['telefono'],
            ...
            data['habeas_data'], data['aceptacion_tratamiento_datos'], data['id']
        ]
...
def add_lead_test_drive():
    ...
    try:
        ...
        if precio_venta is None or precio_venta == "":
            precio_venta = 0
        ...
        with conn.cursor() as cursor:
            query = """
                SELECT nombres, apellidos, identificacion, telefono, direccion, ciudad, correo, fecha_lead, 
                       ...
                       capturado_por, habeas_data, aceptacion_tratamiento_datos, fecha_aceptacion, id, precio_venta
                FROM leads
            ...
            lead_dict = {
                ...
                "id": lead[17],
                "precio_venta": float(lead[18]) if lead[18] is not None else 0  # ✅ siempre numérico
            }
