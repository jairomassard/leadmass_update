<template>
  <div id="gerente-reportes" class="container mt-4">
    <h1 class="text-center mb-4">Dashboard de Reportes - Gerencial</h1>
      ...
      <div class="col-md-6 mb-4">
        <div class="card shadow-sm">
          ...
          <div class="card-body">
            ...
            <div v-else class="text-center text-muted">Sin datos</div>
          </div>
        </div>
      </div>
  ...
  methods: {
    ...
    generarGraficos() {
      ...
      this.chartPipeline = {
        tooltip: { 
          trigger: "axis", 
          formatter: (params) => `${params[0].name}: $${this.formatoCorto(params[0].value)}`
        },
        xAxis: [{ type: "category", data: ["Prospectado", "Real (Ventas)"] }],
        yAxis: [{ 
          type: "value", 
          axisLabel: { formatter: (val) => `$${this.formatoCorto(val)}` }
        }],
        series: [
          {
            type: "bar",
            ...
            label: { 
              show: true, 
              position: "top", 
              formatter: (val) => `$${this.formatoCorto(val.value)}`
            },
          },
        ],
      };
