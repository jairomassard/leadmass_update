...
export default {
  ...
  methods: {
    ...
    guardarLead(lead) {
      ...
      if (!usuarioId) {
        ...
        alert("Error: Usuario no identificado. No se puede guardar el lead.");
        return;
      }

      // Asegurarse de incluir los campos que no son visibles en el formulario
      const leadActualizado = {
        ...lead,
        habeas_data: lead.habeas_data || 'Si', // Preservar el valor actual
        aceptacion_tratamiento_datos: lead.aceptacion_tratamiento_datos || 'Si',
        usuario_id: usuarioId, // Agregar el usuario que realiza la acción
      };

      // Enviar la solicitud al backend
      axios.post('/update-lead', leadActualizado)
        .then(() => {
          alert('Lead actualizado exitosamente');
          this.cargarLeads(); // Refrescar la tabla después de actualizar
        })
        .catch(error => {
          console.error("Error al guardar el lead:", error.response ? error.response.data : error);
    ...
    eliminarLead(leadId) {
      ...
      if (!usuarioId) {
        console.error("No se encontró el usuario_id en localStorage.");
        alert("Error: Usuario no identificado. No se puede eliminar el lead.");
        return;
      }

      if (confirm("¿Estás seguro de que deseas eliminar este lead?")) {
        axios.post('/delete-lead', { lead_id: leadId, usuario_id: usuarioId })
          .then(response => {
            alert(response.data.message);
            this.cargarLeads(); // Refrescar la tabla después de eliminar
          })
          .catch(error => {
            console.error("Error al eliminar el lead:", error.response ? error.response.data : error);
  ...
  created() {
    const usuarioId = localStorage.getItem('id_usuario'); // Verificar el usuario en el localStorage
    if (!usuarioId) {
      alert("Sesión no válida. Por favor, inicia sesión nuevamente.");
      this.$router.push('/'); // Redirigir al login
    } else {
      this.cargarLeads(); // Cargar los leads si la sesión es válida
      this.cargarVendedores(); // Cargar vendedores también
    }
  }
};
</script>
