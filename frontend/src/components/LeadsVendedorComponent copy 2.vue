<template>
  <div id="app" class="container mt-4">
    ...
    <div class="row text-center mb-4">
      <div class="col-md-4">
        <div class="card shadow-sm total-card bg-light">
          <div class="card-body">
            <h5 class="card-title">💵 Proyectado</h5>
      ...
      <div class="col-md-4">
        <div class="card shadow-sm total-card bg-light">
          <div class="card-body">
            <h5 class="card-title">✅ Vendido</h5>
      ...
      <div class="row g-3">
        ...
        <div class="col-4">
          ...
          <input 
            v-if="editMode" 
            type="number" 
            v-model.number="leadActual.precio_venta" 
            class="form-control" 
          />
          <input 
            v-else 
            type="text" 
            :value="formatCurrency(leadActual.precio_venta)" 
            class="form-control" 
            disabled 
          />
        </div>
...
export default {
  ...
  computed: {
    ...
    totalVentas() {
      return this.leads
        .filter(lead => lead.estatus === "culmina en venta")
        .reduce((sum, lead) => sum + (lead.precio_venta || 0), 0);
    },
    totalPerdidos() {
      return this.leads
        .filter(lead => lead.estatus === "cerrado")
        .reduce((sum, lead) => sum + (lead.precio_venta || 0), 0);
    },
    totalProyectado() {
      return this.leads.reduce((sum, lead) => sum + (lead.precio_venta || 0), 0);
    }
  },
  ...
  methods: {
    ...
    obtenerIdConcesionario(nombreConcesionario) {
        ...
        if (!this.listaConcesionarios || this.listaConcesionarios.length === 0) {
            console.error("⚠️ Error: Lista de concesionarios aún no está disponible.");
            return null;
        }
    ...
    consultarLeads() {
      ...
      axios.get(`/get-leads-vendedor?vendedor=${this.vendedorId}`)
        .then(response => {
          console.log("📥 Leads recibidos:", response.data);
    ...
    desformatearFechaColombia(fechaColombia) {
      if (!fechaColombia || fechaColombia.includes("undefined")) {
          console.warn("⚠️ Fecha inválida detectada en el frontend:", fechaColombia);
          return null;
      }
    ...
    cargarConcesionarios() {
        axios.get('/get-concesionarios')
            .then(response => {
                ...
                console.log("✅ Lista de concesionarios cargada:", this.listaConcesionarios);
            })
            .catch(error => {
                console.error("❌ Error al obtener la lista de concesionarios:", error);
            });
    },

    guardarCambios() {
      console.log("📌  Verificando lista de concesionarios antes de guardar:", this.listaConcesionarios);

      // Validar si la lista de concesionarios está disponible antes de continuar
      if (!this.listaConcesionarios || this.listaConcesionarios.length === 0) {
              alert("⚠️ Error: No se puede guardar aún, la lista de concesionarios no ha cargado. Intente nuevamente en unos segundos.");
              return;
          }
      ...
      const leadParaGuardar = {
        ...
        concesionario_id: this.obtenerIdConcesionario(this.leadActual.concesionario_id), // Convertir a ID numérico
        fecha_lead: this.desformatearFechaColombia(this.leadActual.fecha_lead), // Convertir al formato ISO para backend
        usuario_id: this.vendedorId // Agregar el usuario que realiza la acción
      };
      ...
      axios.post('/update-lead', leadParaGuardar)
        .then(() => {
          ...
          this.updateTotalizadores();
          // 🔄 REFRESCAR LEADS ANTES DE SOLICITAR UNO NUEVO
          this.consultarLeads();  // 🔥 IMPORTANTE: Se consulta la BD nuevamente
          setTimeout(() => {
                this.asignarNuevoLead();  // Llamar la asignación después de un pequeño delay
