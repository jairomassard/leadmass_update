...
export default {
  ...
  methods: {
    obtenerUsuarioId() {
        const usuarioId = localStorage.getItem('id_usuario'); // Cambiado a 'id_usuario'
        console.log("Usuario ID en localStorage:", usuarioId);

        if (!usuarioId) {
            console.error("El usuario no está autenticado. No se encontró el ID en localStorage.");
            alert("Por favor, inicie sesión nuevamente.");
            this.$router.push('/login');
            return null; // Asegura que no continúe si no hay usuarioId
        }

        return usuarioId;
    },
    generarContenidoTabla() {
      console.log("Botón presionado: Generar Contenido Tabla");
      this.cargarLeads(); // Llama al método para cargar leads
      this.contenidoGenerado = true; // Activa la tabla
    },
    cargarLeads() {
        ...
        console.log("Enviando usuario_id:", usuarioId); // Agrega un log para confirmar el valor

        axios.get('/get-all-leads-supervisor', { params: { usuario_id: usuarioId } })
            .then(response => {
            ...
            .catch(error => {
              console.error("Error al cargar leads:", error.response ? error.response.data : error);
              alert("Error al cargar los leads: " + (error.response ? error.response.data.error : "Error desconocido."));
            });
    },
    cargarVendedores() {
    ...
    filtrarLeads() {
      ...
      this.actualizarGrafico(); // Actualizar el gráfico después de filtrar
    },
    ...
    exportarReporte(formato) {
      axios.post('/exportar-reporte', {
        ...
        .then(response => {
          ...
          alert("Exportación exitosa. El archivo ha sido generado.");
        })
        .catch(error => {
          console.error("Error al exportar reporte:", error);
          alert("Hubo un error al exportar el reporte. Por favor, intente nuevamente.");
        });
    }
  },
  created() {
