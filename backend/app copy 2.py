    ...
    try:
        ...
        for lead in leads:
            leads_list.append({
                ...
                "ciudad": lead[6],
                "correo": lead[7],
                "fecha_lead": str(lead[8]) if lead[8] else None,
                "origen_lead": lead[9],
                "marca_interes": lead[10],
                "modelo_interesado": lead[11],
                "estatus": lead[12],
                ...
                "fecha_asignacion": str(lead[15]) if lead[15] else None,
                "seguimientos": lead[16] if lead[16] else "",
                "test_drive": lead[17] if lead[17] else "No",
                "capturado_por": lead[18],
                "habeas_data": lead[19],
                "aceptacion_tratamiento_datos": lead[20],
                "firma_digital": lead[21],
                "concesionario_id": lead[22],
                "fecha_aceptacion": str(lead[23]) if lead[23] else None,
                "precio_venta": float(lead[24]) if lead[24] is not None else 0
            })
