...
export default {
  ...
  methods: {
    ...
    guardarCambios() {
      axios.post('/update-lead', this.leadActual)
        .then(() => {
    ...
    async validarArchivoAntesDeEnviar() {
        ...
        const encabezados = filas[0].split(';').map(header => header.trim().toLowerCase().replace(/ /g, '_'));
        const indices = {
            fechaLead: encabezados.indexOf('fecha_lead'),
            habeasData: encabezados.indexOf('habeas_data'),
            tratamientoDatos: encabezados.indexOf('tratamiento_datos'),
        };

        // Verificar si existen los encabezados necesarios
        if (indices.fechaLead === -1 || indices.habeasData === -1 || indices.tratamientoDatos === -1) {
            alert("El archivo no contiene los encabezados necesarios. Verifique la plantilla.");
            return false;
        }
        ...
        for (let i = 1; i < filas.length; i++) {
            ...
            const fechaLead = columnas[indices.fechaLead]?.trim();
            ...
            // Validar Habeas Data
            const habeasData = columnas[indices.habeasData]?.toLowerCase();
            if (habeasData !== 'si' && habeasData !== 'no') {
                alert(`Error en la fila ${i + 1}: El campo 'Habeas Data' debe ser 'Si' o 'No'.`);
                return false;
            }

            // Validar Tratamiento de Datos
            const tratamientoDatos = columnas[indices.tratamientoDatos]?.toLowerCase();
            if (tratamientoDatos !== 'si' && tratamientoDatos !== 'no') {
                alert(`Error en la fila ${i + 1}: El campo 'Tratamiento Datos' debe ser 'Si' o 'No'.`);
                return false;
            }
        }
