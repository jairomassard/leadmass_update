  ...
  <div id="app" class="container mt-5">
    ...
    <div v-if="leadActual" class="card p-4 mb-4">
      ...
      <div class="row">
        ...
        <div class="col-md-4 mb-3" v-if="editMode || leadActual">
...
export default {
  ...
  methods: {
    ...
    async validarArchivoAntesDeEnviar() {
        ...
        const encabezados = filas[0].split(';'); // Primera fila: Encabezados
        const indices = {
            fechaLead: encabezados.indexOf('Fecha Lead'),
            habeasData: encabezados.indexOf('Habeas Data'),
        };

        if (indices.fechaLead === -1 || indices.habeasData === -1) {
        ...
        for (let i = 1; i < filas.length; i++) {
            ...
            // Validar Habeas Data
            const habeasData = columnas[indices.habeasData]?.trim().toLowerCase();
    ...
    async cargarLeadsDesdeArchivo() {
        ...
        axios.post('/upload-leads', formData)
            .then(response => {
                alert(response.data.message); // Mostrar mensaje exitoso
            ...
            .catch(error => {
                if (error.response && error.response.data.error) {
                    ...
                    alert("Ocurrió un error al cargar el archivo. Intente nuevamente.");
                }
            });
    },
    descargarPlantilla() {
