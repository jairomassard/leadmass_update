<template>
    <div id="supervisor-leads" class="container mt-4">
      ...
      <div class="d-flex justify-content-center mb-4">
        ...
        <button class="btn btn-warning me-2" @click="exportarReporte('excel')">Exportar Excel</button>
...
<!-- Tabla de leads -->
        ...
        <tbody>
          <tr v-for="lead in leadsFiltrados" :key="lead.id">
            ...
            <td>
              ...
              <!-- 🔹 Nuevo botón Eliminar con estilo de peligro -->
              <button class="btn btn-outline-danger btn-sm" @click="eliminarLead(lead)">Eliminar</button>
            </td>
          </tr>
        </tbody>
      </table>
  ...
  export default {
    ...
    methods: {
        ...
        eliminarLead(lead) {
          if (
            confirm(`⚠️ ¿Estás seguro de que deseas eliminar el lead de ${lead.nombres} ${lead.apellidos}?`) &&
            confirm("Esta acción es irreversible. Confirma nuevamente para eliminar.")
          ) {
            axios.post('/delete-lead', { 
              lead_id: lead.id, 
              usuario_id: this.usuarioId // 👈 importante: quién lo elimina
            })
            .then(() => {
              alert("Lead eliminado exitosamente");
              this.cargarLeads(); // recargar lista
            })
            .catch(error => console.error("❌ Error al eliminar lead:", error));
          }
        },
      ...
      exportarReporte(formato) {
        const leadsConAsignado = this.leadsFiltrados.map(lead => ({
          ...lead,
          asignado_a: this.filtroVendedor ? parseInt(this.filtroVendedor) : null
        }));
        ...
        .then(response => {
          ...
          link.setAttribute('download', `reporte_leads.${extension}`);
