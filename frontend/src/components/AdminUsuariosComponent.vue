...
export default {
  ...
  methods: {
    consultarConcesionarios() {
      return axios.get('/get-concesionarios')
        .then(response => {
          console.log("✅ Concesionarios cargados:", response.data);
  ...
  async created() {
    await this.consultarConcesionarios();
